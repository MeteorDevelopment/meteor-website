...
- `get_module_setting(id, setting)`: Returns the current value of the setting. 
  Numbers, booleans and strings stay the same type when converted to Starscript. 
  Lists return the number of elements and everything is converted to a string according to it's Java toString() method.
